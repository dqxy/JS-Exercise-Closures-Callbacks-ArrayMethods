...
*/
function processLength(stringList, callback) {
  return callback(stringList.length);
}
 ...
 * should return 1000.
*/
function processSum(nums, callback) {
  const reducer = (accumulator, currentValue) => accumulator + currentValue;
  return callback(nums.reduce(reducer,0));
}
...
function getFullNames(runners) {
  ...
  function getNames(value) {
    ...
    a.push(value.last_name+", "+ value.first_name);
...
function firstNamesAllCaps(runners) {
  let a=[];
  runners.forEach(getNames2); 
  function getNames2(value) {

    a.push(value.first_name.toUpperCase());
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  let a=[];
  runners.forEach(getShirt); 
  function getShirt(value) {

    if(value.shirt_size === tShirtSize){a.push(value)}
...
function counterMaker() {
  ...
  function counter() {
    if (count<= 2) {
      return count++} else if (count > 3) {return count = 0}
  }
  return counter;
  }
  // BROKEN CODE ENDS
